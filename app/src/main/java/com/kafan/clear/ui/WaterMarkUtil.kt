...
object WaterMarkUtil {
    ...
    fun showWatermarkView(activity: Activity, string: String) {
        ...
        framView.findViewById<ImageView>(R.id.ivWm).setImageDrawable(getMarkTextBitmapDrawable(activity, string, width, height))
        //可对水印布局进行初始化操作
    ...
    private fun getMarkTextBitmap(gContext: Context, gText: String, width: Int, height: Int): Bitmap? {
        ...
        val inter: Float = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40f, gContext.resources.displayMetrics)
