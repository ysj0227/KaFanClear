...
class FileUtils {
    ...
    companion object {
        ...
        fun createDir(dirName: String?): File? {
            if (TextUtils.isEmpty(dirName)) return null
        ...
        fun createFile(folderPath: String, fileName: String): File? {
            val filePath: String = SDCardUtil.sDCardPath
                .toString() + File.separator + folderPath + File.separator + fileName
        ...
        fun findAllAPKFile(file: File, lists: MutableList<String>) {
            if (file.isFile) {
                val name = file.name
                if (name.lowercase(Locale.getDefault()).endsWith(".apk")) {
                    val apkPath = file.absolutePath // apk文件的绝对路劲
                    lists.add(apkPath)
                }
            } else {
                if (file.absolutePath.split(File.separator.toRegex()).toTypedArray().size <= 10) {
                    val files = file.listFiles()
                    if (files != null && files.isNotEmpty()) {
                        for (file_str in files) {
                            findAllAPKFile(file_str, lists)
                        }
                    }
                }
            }
